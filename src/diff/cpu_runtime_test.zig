/// diff/cpu_runtime_test.zig: DiffCpuRuntime の数値勾配テスト
///
/// 共通テスト基盤 (test_helpers.zig) を CPU Adapter 経由で使用する。
const std = @import("std");
pub const std_options = @import("../log.zig").std_options;
const compute = @import("../compute.zig");
const Module = compute.Module;
const diff_cpu = @import("cpu_runtime.zig");
...
const CpuAdapter = struct {
    ...
    pub fn make_input(rt: *Runtime, data: []f32, shape: []const usize, requires_grad: bool) Tensor {
        return rt.make_node(data, shape, requires_grad);
    }
...
test "diff_cpu_runtime: negative gradient" {
    ...
    try helpers.test_negative_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: gelu gradient" {
    ...
    try helpers.test_gelu_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: silu gradient" {
    ...
    try helpers.test_silu_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: square gradient" {
    ...
    try helpers.test_square_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: reductionMean gradient" {
    ...
    try helpers.test_reduction_mean_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: tanh gradient" {
    ...
    try helpers.test_tanh_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: sigmoid gradient" {
    ...
    try helpers.test_sigmoid_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: softmax gradient" {
    ...
    try helpers.test_softmax_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: relu gradient" {
    ...
    try helpers.test_relu_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: reductionSum gradient" {
    ...
    try helpers.test_reduction_sum_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: transpose 3D gradient" {
    ...
    try helpers.test_transpose3_d_grad(CpuAdapter, &rt);
}
...
test "diff_cpu_runtime: reshape gradient" {
    ...
    try helpers.test_reshape_grad(CpuAdapter, &rt);
}

// ════════════════════════════════════════════════════════════════
// パラメータ依存テスト (checkGrad を直接使用)
// ════════════════════════════════════════════════════════════════
const check_grad = helpers.gradient_checker(CpuAdapter).check_grad;
...
test "diff_cpu_runtime: mul broadcast gradient" {
    ...
    try check_grad(struct {
        fn f(ctx: *DiffCpuRuntime, x: DiffTensor) DiffTensor {
            const scalar_data = ctx.alloc_data(1);
            scalar_data[0] = 2.5;
            const scalar = ctx.make_node(scalar_data, &.{1}, false);
...
test "diff_cpu_runtime: linear forward+backward" {
    ...
    const input_node = rt.make_node(&input_data, &.{ 2, 3 }, true);
...
test "diff_cpu_runtime: mseLoss gradient" {
    ...
    rt.reset_arena();
    const pred = rt.make_node(&pred_data, &.{ 2, 2 }, true);
...
test "diff_cpu_runtime: crossEntropyLossWithIndices gradient" {
    ...
    rt.reset_arena();
    const logits = rt.make_node(&logits_data, &.{ 2, 3 }, true);
...
test "diff_cpu_runtime: bceLossWithLogits gradient" {
    ...
    rt.reset_arena();
    const logits = rt.make_node(&logits_data, &.{ 2, 2 }, true);
...
test "diff_cpu_runtime: concat last-axis gradient" {
    ...
    try check_grad(struct {
        fn f(ctx: *DiffCpuRuntime, x: DiffTensor) DiffTensor {
            ...
            return ctx.concat_axis(parts[0], parts[1], -1);
        }
    }.f, &rt, &data, &.{ 2, 3 }, 1e-4, 1e-2);
}
...
test "diff_cpu_runtime: concat axis=0 gradient" {
    ...
    try check_grad(struct {
        fn f(ctx: *DiffCpuRuntime, x: DiffTensor) DiffTensor {
            ...
            return ctx.concat_axis(parts[0], parts[1], 0);
        }
...
test "diff_cpu_runtime: split+transform gradient" {
    ...
    try check_grad(struct {
        fn f(ctx: *DiffCpuRuntime, x: DiffTensor) DiffTensor {
            ...
            return ctx.concat_axis(doubled, parts[1], -1);
        }
    }.f, &rt, &data, &.{ 2, 3 }, 1e-4, 1e-2);
}
...
test "diff_cpu_runtime: conv2d gradient" {
    ...
    _ = module.add_param(&.{ 1, 4 }, .xavier);
    _ = module.add_param(&.{1}, .zeros);
    var rt = try DiffCpuRuntime.init(&module, testing.allocator);
    defer rt.deinit();
    ...
    rt.reset_arena();
    const input = rt.make_node(&data, &.{ 1, in_ch * h * w_dim }, true);
...
test "diff_cpu_runtime: maxPool2d gradient" {
    ...
    rt.reset_arena();
    const input = rt.make_node(&data, &.{ 1, 16 }, true);
    const output = rt.max_pool2d(input, 2, 2, 1, 4, 4);
