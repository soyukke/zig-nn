...
pub const TensorInfo = struct {
    ...
    /// 要素数
    pub fn num_elements(self: TensorInfo) u64 {
        var n: u64 = 1;
        for (0..self.n_dims) |i| {
            n *= self.dimensions[i];
        }
...
pub const GGUFFile = struct {
    ...
    /// メタデータを key で検索
    pub fn get_metadata(self: *const GGUFFile, key: []const u8) ?MetadataValue {
    ...
    /// 文字列メタデータ取得
    pub fn get_metadata_string(self: *const GGUFFile, key: []const u8) ?[]const u8 {
        const val = self.get_metadata(key) orelse return null;
        return switch (val) {
            .string => |s| s,
            else => null,
        };
    ...
    /// u32 メタデータ取得
    pub fn get_metadata_u32(self: *const GGUFFile, key: []const u8) ?u32 {
        const val = self.get_metadata(key) orelse return null;
        return switch (val) {
            .uint32 => |v| v,
            else => null,
    ...
    /// u64 メタデータ取得
    pub fn get_metadata_u64(self: *const GGUFFile, key: []const u8) ?u64 {
        const val = self.get_metadata(key) orelse return null;
        return switch (val) {
            .uint64 => |v| v,
            else => null,
    ...
    /// f32 メタデータ取得
    pub fn get_metadata_f32(self: *const GGUFFile, key: []const u8) ?f32 {
        const val = self.get_metadata(key) orelse return null;
        return switch (val) {
            .float32 => |v| v,
            else => null,
    ...
    /// テンソルを名前で検索
    pub fn get_tensor_info(self: *const GGUFFile, name: []const u8) ?TensorInfo {
        for (self.tensors) |t| {
    ...
    pub fn get_tensor_raw_bytes(self: *const GGUFFile, name: []const u8) !RawTensorRef {
        const info = self.get_tensor_info(name) orelse return error.TensorNotFound;
        const n_elem: usize = @intCast(info.num_elements());
        const data_bytes = dequant.tensor_bytes(info.type_, n_elem);
    ...
    pub fn load_tensor_f32(self: *const GGUFFile, name: []const u8, allocator: Allocator) ![]f32 {
        const info = self.get_tensor_info(name) orelse return error.TensorNotFound;
        const n_elem: usize = @intCast(info.num_elements());
        const data_bytes = dequant.tensor_bytes(info.type_, n_elem);
        const file_offset: usize = @intCast(self.data_offset + info.offset);
    ...
    pub fn load_tensor_f32_transposed(
        self: *const GGUFFile,
        name: []const u8,
        allocator: Allocator,
    ) ![]f32 {
        ...
        // まず元データを読み込み
        const raw = try self.load_tensor_f32(name, allocator);
    ...
    /// 配列メタデータの文字列要素を取得
    pub fn get_array_strings(
        self: *const GGUFFile,
        key: []const u8,
        allocator: Allocator,
    ) ![][]const u8 {
        const val = self.get_metadata(key) orelse return error.KeyNotFound;
        ...
        for (0..count) |i| {
            ...
            const str_len: usize = @intCast(read_u64(arr.raw_data[pos..][0..8]));
            pos += 8;
    ...
    pub fn print_info(self: *const GGUFFile) void {
        log.info("GGUF v{d}: {d} tensors, {d} metadata KV", .{
            self.version, self.tensors.len, self.metadata.len,
        });

        if (self.get_metadata_string("general.architecture")) |arch| {
            log.info("  architecture: {s}", .{arch});
        }
        if (self.get_metadata_string("general.name")) |name| {
...
fn read_u32(buf: *const [4]u8) u32 {
    return std.mem.readInt(u32, buf, .little);
}

fn read_u64(buf: *const [8]u8) u64 {
    return std.mem.readInt(u64, buf, .little);
}

fn read_i32(buf: *const [4]u8) i32 {
...
pub fn parse(allocator: Allocator, io: std.Io, file_path: []const u8) !GGUFFile {
    ...
    return parse_from_buffer(allocator, file_buf, file_path);
}
...
fn parse_gguf_header(file_buf: []const u8, pos: *usize) !GgufHeader {
    if (file_buf.len < 20) return error.FileTooSmall;

    const magic = read_u32(file_buf[pos.*..][0..4]);
    pos.* += 4;
    if (magic != GGUF_MAGIC) return error.InvalidMagic;

    const version = read_u32(file_buf[pos.*..][0..4]);
    pos.* += 4;
    if (version < 2 or version > 3) return error.UnsupportedVersion;

    const tensor_count: usize = @intCast(read_u64(file_buf[pos.*..][0..8]));
    pos.* += 8;
    const metadata_kv_count: usize = @intCast(read_u64(file_buf[pos.*..][0..8]));
    pos.* += 8;
...
fn find_alignment(metadata: []const MetadataKV) u64 {
    for (metadata) |kv| {
...
pub fn parse_from_buffer(
    ...
    const header = try parse_gguf_header(file_buf, &pos);
    ...
    for (0..header.metadata_kv_count) |i| {
        const kv = try read_metadata_kv(file_buf, &pos);
        metadata[i] = kv;
    }

    const alignment = find_alignment(metadata);
    ...
    for (0..header.tensor_count) |i| {
        tensors[i] = try read_tensor_info(file_buf, &pos);
    }
...
fn read_gguf_string(buf: []const u8, pos: *usize) ![]const u8 {
    if (pos.* + 8 > buf.len) return error.UnexpectedEof;
    const str_len: usize = @intCast(read_u64(buf[pos.*..][0..8]));
    pos.* += 8;
...
fn read_metadata_kv(buf: []const u8, pos: *usize) !MetadataKV {
    const key = try read_gguf_string(buf, pos);

    if (pos.* + 4 > buf.len) return error.UnexpectedEof;
    const value_type_raw = read_u32(buf[pos.*..][0..4]);
    pos.* += 4;

    const value = try read_metadata_value(buf, pos, value_type_raw);

    return .{ .key = key, .value = value };
}

/// pos の位置から n バイトを読み進められるかチェック
fn require_bytes(buf: []const u8, pos: usize, n: usize) !void {
...
fn consume_bytes(buf: []const u8, pos: *usize, n: usize) MetadataParseError![]const u8 {
    try require_bytes(buf, pos.*, n);
...
fn read_metadata_scalar(
    ...
    switch (vt) {
        .uint8 => {
            const bytes = try consume_bytes(buf, pos, 1);
        ...
        .uint16 => {
            const bytes = try consume_bytes(buf, pos, 2);
        ...
        .uint32 => {
            const bytes = try consume_bytes(buf, pos, 4);
            return .{ .uint32 = read_u32(bytes[0..4]) };
        },
        .int32 => {
            const bytes = try consume_bytes(buf, pos, 4);
            return .{ .int32 = read_i32(bytes[0..4]) };
        },
        .float32 => {
            const bytes = try consume_bytes(buf, pos, 4);
        ...
        .uint64 => {
            const bytes = try consume_bytes(buf, pos, 8);
            return .{ .uint64 = read_u64(bytes[0..8]) };
        },
        .int64 => {
            const bytes = try consume_bytes(buf, pos, 8);
...
fn read_metadata_array(buf: []const u8, pos: *usize) MetadataParseError!MetadataValue {
    if (pos.* + 12 > buf.len) return error.UnexpectedEof;
    const elem_type: ValueType = @enumFromInt(read_u32(buf[pos.*..][0..4]));
    pos.* += 4;
    const count = read_u64(buf[pos.*..][0..8]);
    pos.* += 8;

    const raw_start = pos.*;
    for (0..count) |_| {
        _ = try read_metadata_value(buf, pos, @intFromEnum(elem_type));
    }
    const raw_end = pos.*;
...
fn read_metadata_value(
    ...
    return switch (vt) {
        .string => .{ .string = try read_gguf_string(buf, pos) },
        .array => try read_metadata_array(buf, pos),
        _ => return error.UnsupportedValueType,
        else => try read_metadata_scalar(buf, pos, vt),
    };
}

fn read_tensor_info(buf: []const u8, pos: *usize) !TensorInfo {
    const name = try read_gguf_string(buf, pos);

    if (pos.* + 4 > buf.len) return error.UnexpectedEof;
    const n_dims = read_u32(buf[pos.*..][0..4]);
    pos.* += 4;
    ...
    for (0..n_dims) |i| {
        if (pos.* + 8 > buf.len) return error.UnexpectedEof;
        dims[i] = read_u64(buf[pos.*..][0..8]);
        pos.* += 8;
    }

    if (pos.* + 4 > buf.len) return error.UnexpectedEof;
    const type_raw = read_u32(buf[pos.*..][0..4]);
    pos.* += 4;

    if (pos.* + 8 > buf.len) return error.UnexpectedEof;
    const offset = read_u64(buf[pos.*..][0..8]);
    pos.* += 8;
...
test "parse minimal GGUF" {
    ...
    // Header
    write_u32(&buf, &pos, GGUF_MAGIC);
    write_u32(&buf, &pos, 3); // version
    write_u64(&buf, &pos, 1); // tensor_count
    write_u64(&buf, &pos, 1); // kv_count

    // Metadata KV: key="test.key", uint32, 42
    write_string(&buf, &pos, "test.key");
    write_u32(&buf, &pos, 4); // uint32 type
    write_u32(&buf, &pos, 42);

    // Tensor info: name="w", 1 dim, [4], f32, offset=0
    write_string(&buf, &pos, "w");
    write_u32(&buf, &pos, 1); // n_dims
    write_u64(&buf, &pos, 4); // dim[0]
    ...
    var gguf_file = try parse_from_buffer(alloc, buf_copy, "test.gguf");
    ...
    // Metadata
    const val = gguf_file.get_metadata_u32("test.key");
    try std.testing.expectEqual(@as(?u32, 42), val);

    // Tensor info
    const t = gguf_file.get_tensor_info("w").?;
    ...
    // Load tensor
    const data = try gguf_file.load_tensor_f32("w", alloc);
...
// テスト用のバイナリ書き込みヘルパー
fn write_u32(buf: []u8, pos: *usize, val: u32) void {
    std.mem.writeInt(u32, buf[pos.*..][0..4], val, .little);
    pos.* += 4;
}

fn write_u64(buf: []u8, pos: *usize, val: u64) void {
    std.mem.writeInt(u64, buf[pos.*..][0..8], val, .little);
    pos.* += 8;
}

fn write_string(buf: []u8, pos: *usize, str: []const u8) void {
    write_u64(buf, pos, str.len);
